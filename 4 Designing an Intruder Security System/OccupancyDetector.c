...
int main(void) {
    ...
    WDTCTL = WDTPW | WDTHOLD;   // stop watchdog timer
        ...
        while(1)
        {
            if (!(P4IN & BIT1))            // If button(P4.1) is pressed
            {
                ...
                {                           //do the warning state until 10 seconds pass or button is let go
                    ...
                    __delay_cycles(200000);    // Do this every 2 seconds
                ...
                while (i < 20);
                            ...
                            do                          // do while/ after its been in warning for 10 seconds
                            {
                                P1OUT |= BIT0;          //solid red LED
                                    ...
                                    if (!(P2IN & BIT3))  //if the button is pressed reset while loop
                                    {
                                        i = 0;             //reset i
                        ...
                        {
                            P1OUT &= ~BIT0;
                            P6OUT ^= BIT6;          // Toggle P6.6 / allows the device to get off of armed mode
